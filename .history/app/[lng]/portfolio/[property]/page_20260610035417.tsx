...
export async function generateMetadata({
  ...
  params: { lng: "en" | "fr"; property: string };
}): Promise<Metadata> {
  const { lng, property } = params;
  const propertyPost = await getProperty(property);
  if (!propertyPost) return {};
  ...
  const caption = propertyPost.gallery?.[0]?.caption ?? "";

  return {
    title: `${propertyPost.name} - SCI AFG`,
    description: `${overviewText} ${caption}`,
    openGraph: {
      images: [
        {
          url: urlFor(propertyPost.mainimage)
            .width(1200)
            .height(630)
            .url(),
        },
