...
const HomeHero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // ensure initial positions: first at 0, second at +100%
    xPercentRef.current = 0;
    gsap.set(firstEl, { xPercent: xPercentRef.current });
    gsap.set(secondEl, { xPercent: xPercentRef.current + 100 });

    const speed = 0.1; // tweak to taste

    const loop = () => {
      // advance
      xPercentRef.current += speed * directionRef.current;

      // wrap first between 0 and -100
      ...
      // apply transforms: second always exactly +100 relative to first
      gsap.set(firstEl, { xPercent: xPercentRef.current });
      gsap.set(secondEl, { xPercent: xPercentRef.current + 100 });
