...
const QuickSearch = () => {
  ...
  const [locData, setLocData] = useState("yaounde");
  const [priceData, setPriceData] = useState("0-500000");
  ...
  const hasSelection = useMemo(() => {
    return Boolean(immoData && locData && priceData && areaData && activeTransac);
  }, [immoData, locData, priceData, areaData, activeTransac]);
  ...
  const handleSearch = () => {
    ...
    params.set("transaction", activeTransac);
    params.set("type", immoData);
    params.set("city", locData);
    params.set("price", priceData);
    params.set("area", areaData);
