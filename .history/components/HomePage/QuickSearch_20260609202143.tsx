...
const QuickSearch = ({
  ...
  const [activeTransac, setActiveTransac] = useState("a-louer");
  ...
  const handleSearch = (): void => {
    ...
    if (areaData) params.set("surface", areaData);
      ...
      <div className={`container ${styles.qs__container}`}>
        ...
        <div className={styles.qs__main}>
            ...
            <div className={styles.tran__inner}>
              <div
                className={`${styles.transac} ${
                  activeTransac === "a-louer" ? styles.active : ""
                }`}
                onClick={() => setActiveTransac("a-louer")}
              >
                <span>{t("common:rent")}</span>
              </div>
              <div
                className={`${styles.transac} ${
                  activeTransac === "a-vendre" ? styles.active : ""
                }`}
                onClick={() => setActiveTransac("a-vendre")}
              >
                <span>{t("common:buy")}</span>
              </div>
              <div
                className={`${styles.indicator} ${
                  activeTransac === "a-vendreacheter" ? styles.active : ""
                }`}
              ></div>
            </div>
          </div>
          <div className={styles.others}>
            ...
            <button className={styles.searchbutton2} onClick={handleSearch}>
              <div className={styles.inner}><span>Search</span> <Search /></div>
