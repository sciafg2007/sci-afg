...
const PortfolioBottom = ({
  ...
  const buildFiltersFromParams = () => {
    ...
    const cityRaw = searchParams.get("ville");
    const priceRaw = searchParams.get("prix");
    const areaRaw = searchParams.get("surface");
  ...
  const buildGROQ = (filters: ReturnType<typeof buildFiltersFromParams>) => {
    ...
    if (selectedSort === 0) order = " | order(publishedAt desc)";
    if (selectedSort === 1) order = " | order(price asc)";
    if (selectedSort === 2) order = " | order(price desc)";
...
name,
  ...
  const buildPrefilledMessage = (): string => {
    ...
    const cityRaw = parseCommaParam2(searchParams?.get("ville"));
    const priceRaw = parseCommaParam2(searchParams?.get("prix"));
    const areaRaw = parseCommaParam2(searchParams?.get("surface"));
        ...
        <div className={`container ${styles.pb__container}`}>
          <div className={styles.pb__sortfilter}>
            <div className={styles.filter__wrapper}>
              ...
              {activeFiltersCount > 0 && (
                <div
                  className={styles.cancel__filter}
                  onClick={clearAllFilters}
                >
                  <span className={styles.f__name}>
                    {t("portfolio:clear")} ({activeFiltersCount})
                  </span>
            ...
            <div className={styles.sort__wrapper}>
              ...
              <div className={styles.s__right}>
                ...
                >
                  <span className={styles.f__name}>
                    {selectedSort === 0
                      ? t("portfolio:mostrecent")
                      : selectedSort === 1
                        ? t("portfolio:priceup")
                        : selectedSort === 2
                          ? t("portfolio:pricedown")
                          : t("portfolio:mostrecent")}
                  </span>
                ...
                <div
                  className={`${styles.sort__overflow} ${
          ...
          <div className={styles.pb__content}>
            {loading ? (
              ...
              <div className={styles.properties}>
                {properties.map((p: PropertyProps) => {
                  return <ProjectBox projectDetails={p} key={p._id} />;
                })}
              </div>
            )}
          </div>
