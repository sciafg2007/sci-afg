...
const ProjectBox = ({ projectDetails }: { projectDetails: PropertyProps }) => {
  ...
  const pricingLabels = {
    en: { perday: "perDay", perweek: "perWeek", perMonth: "perMonth" },
    fr: { perday: "perJour", perweek: "perSemaine", perMonth: "perMois" }
  };
  
  // Default to English or an empty string if the key doesn't exist to prevent crashes
  const currentLabel = pricingLabels[currentLocale]?.[projectDetails.rentpricing] || ""
